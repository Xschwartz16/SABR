...
With a known number of components, mixture models use an expectation-maximization algorithm to help identify potential parameters for the different distributions. The expectation-maximization works similarly to k-means by initially assigning samples of the data to "k" (a user input value that we will select) to k mixtures and then assuming that each remaining observation has a uniform likelihood of belonging to each mixture. Next, the probability of each remaining observation belonging to each mixture is found, using probability theory. Finally, each mixture's distribution is updated by maximizing the expected conditional log-likelihood with respect to the assumed distribution [@melnykov_maitra_2010]. For multivariate mixture modeling, the same principles apply as univariate mixture modeling, but use a covariance matrix for each observation for assumed distributions to define the relationship between variables [@nagode_fajdiga_2011].

When discussing multivariate mixture models, the shapes of the models must also be defined in terms of the orientation of the eigenvectors of the mixtures. To simplify the problem we can add constraints to the groupings that we look for in the covariance matrix. This can allow us to look at fewer parameters to estimate and provide easier interpretations of the mixture outputs. The different parameters can help to define how the output mixtures look. For example, when constraining volume we can define whether we want the volumes to be roughly similar in each mixture, or if we are willing for them to be wildly different (see *EEI* vs *VII*). Similarly, we can define if we want the shapes to be roughly similar, or if they can be different (see *EEI* vs *VEI*). Finally, we can define the direction of orientation of our mixtures (see *EEI* vs *EEV* vs *EEE*) [@hennig_2011]. For each potential mixture, the volume (equal, varying), shape (equal, varying, or round), and type of covariance matrix (spherical, diagonal, or ellipsoidal) must be defined, as each potential combination will produce varying degrees of mixture strength, which can be measured using Bayes Information Criterion (BIC). Like a scree plot, it is common for all potential combinations to be tested to find the combination that produces the best mixture. An example of each combination can be seen below [@nugent_2009]. 
...
In short, due to the small sample sizes, it can be difficult to make meaningful claims from just a batter's history against a given pitcher, such as that a batter hits a specific pitcher well. If we assume that the results of a series of at-bats follow a binomial distribution with true probability $p$, even if a batter is 4 for 9 against a pitcher, good for a .444 batting line, this does not indicate a strong skill against that pitcher. If the true expected batting average against that pitcher for this batter is .250, that batter will get 4 or more hits almost 17% of the time per 9 at-bats. Likewise, the batter will get no hits or just one hit over 30% of the time. The sample sizes are simply too small to make any claim of value for most situations. Furthermore, we can view this study as a way of narrowing our binomial test confidence interval by expanding our sample size, while also looking to balance the tradeoff between increasing sample sizes and decreasing the value of the information, as the greater our grouping sizes are (and thus the greater our sample sizes would be), the less similar each pitcher will be.
```{r, eval=FALSE}
1 - pbinom(3, 9, .25)
dbinom(0, 9, .25) + dbinom(1, 9, .25)
```
In this study, we seek to remedy this problem by increasing the sample sizes. Instead of looking at just a batter's history versus a given pitcher, this study looks at a batter's history against a given *type of* pitcher. To increase sample sizes, we take advantage of all pitchers being unique, but some pitchers being more unique than others. From the way they [pitch](https://twitter.com/PitchingNinja/status/1436871406382354432), to the pitches they [throw](https://twitter.com/PitchingNinja/status/1458851989933240326), to how they [utilize](https://twitter.com/pitchingninja/status/1399927522549190659?lang=en) their various pitches, each pitcher's pitching style tells a story awash in data. This data can be utilized to group pitchers together. Sometimes, two pitchers may have similar pitching styles purely [coincidentally](https://community.fangraphs.com/two-of-the-most-similar-pitchers-in-baseball/), and other times a pitcher might model himself after another pitcher. 

One such example is the story of Roy Halladay's cutter [@kepner_2019]. Famously, Roy Halladay learned his cutter from fellow hall-of-famer Mariano Rivera at an [All-Star Game](https://www.nytimes.com/2019/01/20/sports/hall-of-fame-roy-halladay.html), so unsurprisingly, their cutters would have similar attributes in spin rate and velocity, as they were intentionally trying to throw identical pitches. By grouping pitchers together according to their similar attributes, the sample sizes can increase enough that we can draw meaningful conclusions about how a batter does against a given type of pitcher. Say a batter has just 2 plate appearances against a pitcher, but 70 plate appearances against that type of pitcher, those 70 plate appearances might hold predictive value in projecting the at-bat outcome. This study aims to test that assertion.

### Past Baseball Work

Numerous past studies have looked to measure the value of batter-pitcher matchup history. Renowned baseball statistician Tom Tango first looked at this in a 2006 study, finding that he could not conclude that batter-pitcher matchup history held any meaning in predicting future outcomes [@tango_lichtman_dolphin_2014]. Former San Diego Padres analyst Dave Cameron took another approach to argue against batter-pitcher history. He uses the example of the Mariners starting 41-year-old Raúl Ibañez (a very old age for a professional baseball player) in 2013 against Yankees ace C.C. Sabathia. Ibañez had very good career numbers against Sabathia in 52 plate appearances, hence why the Mariners chose to play Ibañez that day, despite his normally not starting. As Cameron further breaks down Ibañez's history against Sabathia, he notes that much of Ibañez's success came in 2002 and 2003, and was fairly mediocre against Sabathia in the years afterward. In 2002, Ibañez was in his prime as an above-average outfielder, whereas Sabathia was 21 years old (a very young age for a professional baseball player), in his second year in the majors, still learning how to pitch at a Major League level, and a much worse pitcher than he was in 2013. While Ibañez undoubtedly had plenty of experience against Sabathia, the notion that Ibañez's history over ten years ago would be relevant against Sabathia in 2013, despite Ibañez's lack of recent success, demonstrates that even extensive batter-pitcher matchup histories oftentimes still do not hold predictive merit. [@cameron_2013]

Other studies have also endeavored to group pitchers together. Some have analyzed pitchers by examining their outcomes, while others have analyzed pitchers using their pitch characteristics. Studying outcomes helps to answer a different question: how does a batter do against a specific *skill level* of the pitcher? For example, one batter may do well (relative to the rest of the league) against top pitchers, while another batter may struggle against the best pitchers more than usual and hammer the worst pitchers [@sarris_2020]. Other studies of pitch clustering used pitchf/x data (a previous high-speed camera pitch classification system with less detailed data) to cluster pitchers, although the studies did not draw any conclusions about the value of clustered pitcher groups [@zhao_healey_brooks_2017].

## Hypothesis

Prior to beginning the data analysis for this study, this paper hypothesized that it could be shown that looking at groups of similar pitchers can have predictive value in projecting the outcome of batter-pitcher matchups. From a modeling perspective, we also expect the mixture model to be more successful in creating predictive groups of pitchers than the more traditional k-means projections.

# Methods

## Data Collection

The entirety of this data was scraped from [Baseball Savant](baseballsavant.mlb.com) using the `baseballr` package. The original dataset consisted of every pitch in the Major League Baseball regular season from 2015 to 2019. These years were chosen, as at the time of data collection these were all past full seasons with Statcast data available. Each pitcher who threw at least 100 pitches of each of their pitch types over the 5 seasons was included in the dataset. Due to small sample sizes, eephuses, knuckleballs, forkballs, and screwballs were not included, as no more than 4 total pitchers in the entire dataset threw any of those pitches. In the end, there were 1204 pitchers in the dataset. 

For modeling pitch outcomes, each at-bat from 2015-2019 was included. This means that only plate appearances ending in an out or hit were kept in the dataset. For modeling purposes, only at-bats in which both the hitter had more than 30 plate appearances and the pitcher had more than 30 batters faced, were kept. Using this methodology, 925,098 at-bats were kept. 

## Methods Overview

To best measure the validity of different grouping approaches and to test our baseball hypothesis, there are a series of steps to take. Firstly, the pitchers need to be separated into groups. To group the pitchers via k-means clustering and mixture modeling, the number of groups for each method should be determined. Ideally, the group count of both methods is relatively similar, so that comparison can be more easily made and have similar values. If this were not true, we could end up with a situation where one algorithm has two groups - perhaps one for righties and one for lefties, and the other algorithm might produce 60 groups, making it an exercise in futility to compare the algorithms' pros and cons. Once the groupings are complete, comparable models must be created using the various groupings (control, k-means, mixture). Each model should account for the differences in sample sizes between players. A player who is 30/100 against a cluster or mixture versus 3/10 should have different weights, as the 3/10 player should have more shrinkage and regression to the mean, whereas the player who has hit 30/100 should more heavily weight that data. Finally, the models should be compared to test the baseball hypothesis and the strengths and weaknesses of each grouping algorithm.

## Preliminary Analysis of Dataset

To best understand the outputs, we should have some understanding of our data. The first thing to look at is the overall pitch rates in the dataset. It would be unfair to characterize this breakdown as what the average pitcher throws in the dataset, as no pitcher throws all of the pitches. Furthermore, despite splitters only making up 1.13% of the overall dataset of all pitches thrown from 2015-2019, some pitchers still throw a heavy rate of splitters. For four pitchers, Zach Putnam, Zac Grotz, Oliver Drake, and Hector Neris, splitters make up a majority of their pitches, hence why it is important to keep pitches such as splitters and knuckle-curveballs in the dataset, even if those pitches make up only a small percentage of the overall dataset. This distribution can be seen in the table below. The first column shows the rate of pitchers who throw each pitch. The second column shows the overall frequency of each pitch among all pitches in the dataset. The third column shows the mean frequency of each pitch, among all pitchers who throw that pitch. For example, 23.5% of pitchers throw a sinker, but only 10.7% of all pitches are sinkers. However, among the 23.5% of pitchers who throw a sinker, those pitchers throw 45.6% sinkers on average.  
...
df%>%kable(col.names = c("Pitch Type", "% of Pitchers", "% of All Pitches", "% Pitches Among Pitchers"),caption = "Pitch Rates",font_size = 8)%>%
...
Digging further into the numbers, velocity is a key component of each pitch. Each pitch type is, on average, thrown at a  different speed. While many pitches may be thrown at max effort, having velocity differentials between pitch types is an important and intentional part of pitch design. For example, the main reason a pitcher would throw a changeup is to have a natural pair to their fastball in so far as the changeup is meant to look like a fastball but with a lower velocity so that a hitter will swing too early and miss the changeup, as they thought the pitch was a fastball. When looking at the barplot, the means are calculated by removing any pitcher who does not throw that specific pitch from the mean calculation; whereas in the violin plot, a pitcher who does not throw the pitch is calculated as a 0. 
...
Next, we will analyze spin rates, which track how the ball moves through the air. Spin rates, calculated using the revolutions per minute of the ball, have a large effect on the total movement and break of an individual pitch, and pitchers generally want higher spin rates on their pitches. Like the velocity plots, when looking at the barplot, the means are calculated by removing any pitcher who does not throw the pitch from the mean calculation; whereas in the violin plot, a pitcher who does not throw the pitch is calculated as a 0. 
...
The last, and potentially most tricky part of the dataset is the release extensions. In this dataset, extension is defined as the total Euclidian distance from the center of the mound in terms of vertical and horizontal height. Righthanders were programmed as negative extension and lefthanders were programmed as positive extension. The extension was measured for each pitch, as there can be pitch-to-pitch variation for some pitchers, but for the most part, each pitcher generally maintains a similar release point, hence the scatterplots of any two pitches mapped against each other below. For pitchers who do not throw a specific pitch, their release point for that pitch is 0. 
```{r, fig.height=3.5, fig.width=6, fig.cap= "A Look At Release Extensions"}
extensions <- final_pitcher_data %>% select(20:28)
colnames(extensions) <- c("2FB", "4FB", "CHU", "SLD", "SNK", "KCV", "CRV", "CUT", "SPL")
GGally::ggpairs(extensions, upper = list(continuous = wrap("cor", size = 2)))
```
Like the other statistics, the distributions are relatively normal, with two caveats. Firstly, as the density plots show, much of the density is for players who do not throw the pitch (more density towards 0 indicates that the pitch is less common), and secondly, the distributions are bimodal due to the righties and lefties. If the extension data did not include 0, and we used the absolute value of the extension, so that righties and lefties were treated the same, then it would be fairly normal. This can be seen in a histogram showing the average extension on 4-seam fastballs. The low correlations are partially a product of pitchers who do not throw a pitch having an extension of 0, and the difference between righties and lefties.
```{r, fig.height=2, fig.width=3.5, fig.cap="Comparing Righties to Lefties"}
...
```{r, fig.height=2.5, fig.width=7, out.width="100%", fig.cap="How Many Clusters?", out.height="18%"}
...
#kClust + geom_vline(xintercept = 48, colour = "light blue", linetype = "longdash")
knitr::include_graphics("otherObjects/clusterPhoto.png")

```

Having now created the clusters, we employed a decision tree to help explain what is happening under the hood of the cluster generation, although the decision tree is only an approximate look at the clustering and does not fully account for every cluster nor perfectly classify every pitcher. To walk through the cluster generation, the clustering algorithm first recognizes that a cluster can be created of pitchers who throw almost exclusively fastballs. From there, pitchers are divided by whether they throw a 2-seam fastball. Next, pitchers are clustered by aspects of their other potential pitches, such as the spin rate on their curveball, or the velocity of their slider. On average, there are about 25 pitchers per cluster, with a maximum of 107 and a minimum of 4. Cluster 8, which has 107 pitchers, includes primarily righties who frequently throw fastballs and occasionally throw splitters. Cluster 8 is primarily middle relievers and lacks any pitchers with all-star appearances with the exception of R.A. Dickey. Dickey is likely mischaracterized as he throws primarily a knuckleball and knuckleballs were removed from the dataset, due to Dickey and Steven Wright being the only pitchers with substantial MLB experience who throw knuckleballs. Cluster 43 is made up of four righty relievers who throw mostly sinkers, 2-seam fastballs, and sliders, as well as the occasional 4-seam fastball. This is a well-defined cluster without outliers, whereas cluster 8 includes some pitchers who roughly fit the general description of cluster 8, but might have one or two different attributes that are not commonly found in cluster 8.
...
The same basic principles that are used in clustering apply to mixture model group creation. Like clustering, we want to test for some large number of potential mixture numbers and then select the mixture count that creates the strongest mixtures. To do this, we will use BIC, looking for the lowest score [@mclachlan_peel_2000, 209]. Below is a plot showing the BIC scores for different potential mixture algorithms. This plot is similar to a k-means scree plot but is also testing different mixture model constraints, looking for both the best BIC and constraint set. As mentioned above, each potential option provides different constraints to the potential mixture model distributions. We will use *EEI* (equal volume, round shape, spherical covariance) as our mixture algorithm with 38 mixtures. 
```{r, results='hide',fig.keep='all', fig.cap="How Many Mixtures?", fig.height = 4}
load("otherObjects/mixCluster.rda")
plot(hold)[1] + abline(v = 38)
```
Looking at the now-created mixtures, the mixtures seem mostly focused on handedness, before any other attribute. Using a decision tree once again, we can see that the main focus of the mixture model is to first identify the pitchers that throw the two least common pitches - splitters and knuckle curves. The mixture model quickly separates the righties who throw those pitches into their own groups. It seems like the biggest splits are whether the pitcher is a righty who throws changeups or a lefty who throws sliders. Almost all the splits look to use righty and lefty extensions, grouping the pitchers by handedness and pitch types. The groups average about 32 pitchers per mixture, with a maximum of 90 and a minimum of 4. The largest mixture grouping, mixture 36, contains almost exclusively above-average righty relievers who rely exclusively on their fastball and slider but are not typically the best relievers in the game. Two mixture groupings have just four pitchers. One of these mixtures is mixture 24, which might be the most interesting mixture in the entire dataset. The four "pitchers" include LaTroy Hawkins, Kevin Gregg, R. A. Dickey, and Chris Gimenez. These players average the birth year of 1976 (given the first year of data being used, the players averaged being 39 years old at the earliest possible time of data collection, which would place all of these pitchers among the oldest pitchers in the dataset) and all throw exclusively fastballs, although once again, Dickey is mischaracterized due to his knuckleballs not being counted. Notably, Chris Gimenez was a major league catcher but pitched enough in late-game blowouts to meet the minimum qualifications for this dataset (his fastball averaged 69mph).  
...
```
Finally, mixture models produce probabilistic groupings. This means that a player could have a 30% chance of belonging to one mixture and a 70% chance of belonging to another. Interestingly, there were only six pitchers who had less than a 95% chance of belonging to one mixture and a greater than 5% chance of belonging to a different mixture. All of these pitchers were likely to be a member of mixture 5, 10, or 32. Each of those three mixtures involved entirely righties who throw fastballs and cutters, and occasionally sliders. Each of these six pitchers throws a fastball and cutter, and one pitcher throws a slider. All of the pitchers were eventually assigned to mixture 10, except for the pitcher who throws a slider who was assigned to mixture 32. 
...
```{r, fig.height=2.25, fig.cap = "A Look At Group Sizes"}
mixes2 <- mixes %>%
...
```
To take a look at one pitcher, we will look at Max Scherzer and his cluster and mixture. Scherzer, one of the best pitchers in baseball, boasts a repertoire consisting of a 4-seam fastball, 2-seam fastball, cutter, changeup, curveball, and slider, while generally throwing primarily his 4-seam fastball with even mixes of his other pitches. Scherzer is right-handed and averages 93mph on his fastball. Scherzer belongs to cluster 13 and mixture 14. There are 25 pitchers who overlap in both cluster 13 and mixture 14. Mixture 14 has 39 pitchers, all righties, who each throw a 4-seam fastball, 2-seam fastball, cutter, changeup, and curveball. The majority throw a slider, although relatively infrequently. All of the pitchers throw solid fastballs, but not extremely fast pitches, ranging from about 90mph to 96mph and with wider velocity variations in their offspeed pitches. Cluster 13 is made up of 41 pitchers, 31 of whom are right-handed and ten of whom are left-handed. Pitchers in cluster 13 all throw a 4-seam fastball, 2-seam fastball, cutter, slider, and curveball, while some pitchers throw splitters and changeups. Almost every pitcher throws a majority of his pitches as fastballs (a mix of 4-seam and 2-seam). In comparing the two methods, the within-group velocities variance is relatively similar, although cluster 13 has less variance in its spin rates than mixture 14. Both algorithms produced groups with pitchers with similar pitch types. In short, it seems that the clustering algorithm traded having groupings with exclusive handedness, for groups with more similar pitches even within the pitch type. 

## Results Preview

To test the strength of the grouping algorithms, a variety of methods will be used to compare three different inputs: batter versus pitcher history, batter versus cluster history, and batter versus mixture history. Mathematically, batter versus pitcher history could be considered the same as treating each observation as their own mixture or cluster and helps to serve as a baseline model against which to compare the other models. To help account for the differences in sample sizes, a beta-binomial distribution, using Bayesian inference, will be used to help produce a future expected batting average. A beta-binomial distribution uses a prior that a given value follows a beta distribution and then takes the outcome of a binomial situation to create a posterior distribution. More formally, the beta-binomial distribution shows that if $X \sim \operatorname{Beta}(\alpha, \beta)$, and we are given $Y \sim \operatorname{Bin}(n,p)$, then $X|Y \sim \operatorname{Beta}(\alpha +np, \beta+n(1-p))$. For predicting future batting averages, we will use a prior that $BA \sim \operatorname{Beta}(14, 34)$, which may seem to be an arbitrary choice (and it is), but the $\operatorname{Beta}(14, 34)$ distribution closely follows the batting average distribution of our dataset, as seen below for each batter in the dataset. It may seem like a $\operatorname{Beta}(14, 34)$ distribution produces too high of a median batting average, but the batting average of this dataset is much higher than the overall MLB, as only players who meet a minimum at-bat threshold are kept in the dataset, which increases the quality of batters. To give an example of how the distribution would play out, the binomial information given would be a player's history against a pitcher, cluster, or mixture. Say a batter was 35/100 against a given pitcher's cluster. The posterior would now follow a $\operatorname{Beta}(14+35, 34+100-35)=\operatorname{Beta}(49,99)$ distribution. This example can be seen below on the right, with the black line showing the new information of a .350 batting average in 100 at-bats. 
...
To best understand the results, we will build three separate logistic regression models (using an 80-20 training-testing split), with overlapping inputs, each meant to predict the outcome of an individual at-bat between a specific batter and pitcher. Each model will take in the pitcher's season batting average against (the batting average the pitcher allows to opposing hitters), the batter's season batting average, and the batter's history against the grouping method that the model is testing. For each of the different inputs, including the batting averages, the result of the plate appearance being modeled has been removed so that the model does not mistake a situation in which a batter is 5/5 against a pitcher as having a 100% likelihood that the current at-bat would also result in a hit. From there, a variety of tests will be performed to compare the models. 
...
caret::varImp(bothMod)
```
```{r, fig.width=6.5, fig.height=2.25, fig.cap="Predictions By Grouping Method"}
c <- ggplot(test_predicted_data, aes(x = isHitC, fill = as.factor(isHit))) +
  geom_density(alpha = .4) +
  xlim(.175, .4) +
  xlab("Predicted Batting Average Via Clustering") +
...
The results of this study illuminate the differences in the strengths and weaknesses of mixture models versus clusters. In this study, mixture models clearly outperformed cluster models and more closely matched the baseball intuition that handedness should be an important factor in grouping pitchers. Both models outperformed the baseline of treating every observation as their own individual group, which supports the notion that grouping via clustering and mixture models can improve sample size quality, in small sample situations. As mentioned in the introduction, the mean at-bats between batter and pitcher in this dataset is 2.25, however, the mean at-bats between pitcher and cluster is 17.08 and the mean at-bats between pitcher and mixture is 20.96. The distributions can be seen below (note the different y-axis scales). 
...
While this data set produces mixtures in which almost every observation fits neatly into a specific group, one of the key strengths of the mixture models is the probabilistic nature of the groupings. While formulating the question for this study, one of the expected challenges was that past histories against a given mixture were going to need to be weighted by the likelihood of an observation belonging to each mixture. This problem could have become quite circular as if most observations do not clearly belong to one mixture, what does it mean to have history against a given mixture? One potential solution could be to assign each observation to its most likely mixture, followed by a second round using the probabilistic weights to adjust the past histories against the given most likely mixtures. A future study with a dataset that produces less clear mixtures would provide another opportunity to test the strengths of mixture modeling versus k-means clustering, as k-means does not compute probabilistic groupings, and instead assigns each observation to the cluster of the closest centroid. While this method is quick and dirty, while remaining relatively effective, a mixture model can better deal with the observations that are split between two k-means centroids.

### Limitations & Assumptions

There are a variety of limitations that should be mentioned for how this study can be extrapolated. First and foremost, while this study may have many theoretical implications, the study's findings can only fully be applied to this baseball dataset towards this baseball question. We cannot say that mixture models should always be preferred to k-means clustering when grouping observations. Similarly, the k-means and mixture model algorithm would preferably have the same number of groups, which would allow for more direct comparison. This is important because if you were to compare one algorithm that used two groups against an algorithm of 100 groups, the comparison would be relatively useless.^[In baseball, this might be comparing the value of all righties versus all lefties against a model that produces extremely specific groupings of very few pitchers.] Furthermore, mixture models rely upon the data following some set of distributions, whereas k-means clustering makes no such assumptions. This study also relies upon certain assumptions made by the researchers, such as setting a minimum number of plate appearances in a season to be able to stay in the dataset, which could affect the outcomes of the grouping methods. 
...
The next obvious extension is to test the same premise beyond just simple batting averages. While batting average is important, looking at more advanced baseball statistics would either enhance or show the limitations of the study. Almost any statistic that is the result of a batter-pitcher matchup could be measured by the grouping method and could test the same premise. A simple extension could be to project on-base percentage, which, at a minimum, would allow us to use all plate appearances, not just at-bats. 

Finally, the reverse of this study would also be interesting to test: do pitchers pitch well against a certain *batter type*? Are there commonalities among certain batters that some pitchers could exploit or struggle against? Intuitively, batters seem less likely than pitchers to be easily able to be grouped, as pitchers have more varied data points such as different pitch mixes and velocities. Outside of batter-handedness^[Finding the best methodology to group switch-hitters would provide an interesting challenge!] and swing path, it seems like there are fewer characteristics about batters that make them statistically unique. One potential method could be to group batters by what types of pitches they hit well, although this would likely produce inverse results to this study that would provide very similar values. 
