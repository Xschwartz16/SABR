...
ID_lookup <- baseballr::get_chadwick_lu()%>% filter(mlb_played_last>=2015) %>% mutate(name = paste(name_first, name_last))%>% select(name, key_mlbam)
```
...
```{r}
filteredData <- all_info_cluster %>% filter(P_AB > 5 & C_AB > 5) %>% drop_na()
